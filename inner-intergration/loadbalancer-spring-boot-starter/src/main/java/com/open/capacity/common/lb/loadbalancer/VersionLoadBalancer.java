...
public class VersionLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    private String getVersionFromRequestData(RequestData requestData){
        ...
        if(MapUtils.isNotEmpty(queryMap)&& queryMap.containsKey(CommonConstant.O_C_P_VERSION)&& StringUtils.isNotBlank(queryMap.get(CommonConstant.O_C_P_VERSION))){
            return queryMap.get(CommonConstant.O_C_P_VERSION);
        }else if(requestData.getHeaders().containsKey(CommonConstant.O_C_P_VERSION)){
            return requestData.getHeaders().get(CommonConstant.O_C_P_VERSION).get(0);
        }
        return null;
    }
    /**
